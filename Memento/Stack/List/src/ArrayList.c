...
#elif Use_Array_Int || Use_Array_Ref
...
static inline int indexOfRange(struct ArrayList *obj, dataType data, int start, int end) {
    dataType *es = obj->array;
    for (int i = start; i < end; ++i) {
#if Use_Array_Float
        if (fabsf(es[i] - data) < 0.00000001f){
            return i;
        }
#else
        if (es[i] == data) {
 ...
 * or -1 if there is no such index.
 */
static inline int lastIndexOf(struct ArrayList *obj, dataType data){
    return obj->lastIndexOfRange(obj,data,0,obj->size);
}

static inline int lastIndexOfRange(struct ArrayList *obj, dataType data, int start, int end){
    dataType *es = obj->array;
...
static inline boolean remove(struct ArrayList *obj, dataType data) {
    ...
    int i = 0;
    for (; i < size; ++i) {
#if Use_Array_Int || Use_Array_Ref
...
static inline ArrayList *clone(ArrayList *obj) {
    ...
    memcpy(copy->array, obj->array, obj->size * sizeof(dataType));
    copy->size = obj->size;
    return copy;
}
#if Use_Array_Ref
...
ArrayList *createArrayList(int capacity) {
    ...
    arrayList->lastIndexOf = lastIndexOf;
    arrayList->lastIndexOfRange = lastIndexOfRange;
